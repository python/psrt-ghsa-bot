"""Integration tests for Playwright client (hit live GitHub)."""
...
@pytest.mark.skip(reason="Manual test - run explicitly to set up initial authentication")
def test_manual_authentication() -> None:
    ...
    Only really for localdev because we want CI to be automagic so use PAT for that.

    Run with: pytest tests/integration/test_playwright.py::test_manual_authentication -v
    """
    with GitHubPlaywrightClient(headless=False) as client:
        client.authenticate_manual(timeout=120000)
        assert client._is_authenticated()
